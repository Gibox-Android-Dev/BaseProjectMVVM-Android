...
class ObjectGraphic(
   ...
   /* private val boxPaints = Array(numColors) {Paint()}
    ...
    private var boxPaints : Paint = Paint()
    private var textPaints : Paint = Paint()
    private var labelPaints : Paint = Paint()

    init {
        boxPaints.color = Color.WHITE
        boxPaints.style = Paint.Style.FILL
        boxPaints.alpha = 70

        textPaints = Paint()
        textPaints.color = Color.WHITE
        textPaints.textSize = 54.0f
    ...
    override fun draw(canvas: Canvas) {
        ...
        canvas.drawRoundRect(rect, 8f, 8f, boxPaints)
        // Draws object text.
