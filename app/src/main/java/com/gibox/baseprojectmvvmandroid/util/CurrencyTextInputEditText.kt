...
class CurrencyTextInputEditText @JvmOverloads constructor(
    context: Context?,
    ...
    private class CurrencyTextWatcher internal constructor(
        private val editText: CurrencyTextInputEditText,
        private val prefix: String
    ) :
        TextWatcher {
        private var previousNumber: String? = null
        var integerFormatter: DecimalFormat
        ...
        private fun formatInteger(str: String): String {
            val parsed = BigDecimal(str.replace("[^0-9.,]*".toRegex(), ""))
        ...
        private fun handleSelection() {
            if (editText.text != null) {
                editText.setSelection(Math.min(editText.text!!.length, MAX_LENGTH))
            }
        }
        ...
        init {
            integerFormatter = DecimalFormat(
                "#,###.###", DecimalFormatSymbols(Locale.forLanguageTag("ID"))
            )
        }
    }
